<template>
  <div v-show=hidden>
    <div class="w-full h-full flex items-center justify-center fixed z-20">
      <div v-bind:style="styles"
           class="spinner spinner--hour-glass">
        <div v-bind:style="innerStyles"
             class="spinner-inner">
          <svg class="hourglass"
    ...
    <div class="bg-transparent w-full h-full fixed z-10"></div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.$root.Bus.$on("loading", () => this.hidden=true);

    this.$root.Bus.$on("loaded",()=>this.hidden=false)
  }
};
</script>
...
.hourglass {
  ...
  width: 32px;
  height: 64px;
  box-shadow: inset $bg 0 0 0 0, inset $fill 0 32px 0 0, inset $bg 0 0 64px 0;
...
@keyframes hourglass {
  0% {
    ...
    box-shadow: inset $bg 0 0 0 0, inset $fill 0 32px 0 0, inset $bg 0 64px 0 0,
      inset $fill 0 64px 0 0;
  }
  80% {
    transform: rotate(0deg);
    box-shadow: inset $bg 0 32px 0 0, inset $fill 0 32px 0 0,
      inset $bg 0 32px 0 0, inset $fill 0 64px 0 0;
  }
  100% {
